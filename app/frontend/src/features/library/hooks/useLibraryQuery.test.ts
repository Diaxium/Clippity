...
import type { CapturePage, LibraryQuery } from "@services/tauri/clients/library";
...
describe("useLibraryQuery", () => {
  ...
  it("appends the next page on loadMore and stops at total", async () => {
    ...
    expect(libraryQueryMock).toHaveBeenLastCalledWith({ limit: 100, offset: 100 });
  ...
  it("reloads from the top on library/updated", async () => {
    ...
    expect(libraryQueryMock).toHaveBeenLastCalledWith({ limit: 100, offset: 0 });
  });
